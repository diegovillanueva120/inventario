...
function addSociete($nom, $adresse, $logo, $ext_logo)
{
    ...
    $imageWidth = $imageInfo[0];  // Width in pixels
    $imageHeight = $imageInfo[1]; // Height in pixels
    
    // Set the width you want (25 in this case), calculate the height based on the aspect ratio
    $width = 25;
    $height = ($imageHeight / $imageWidth) * $width;

    // Positioning and output the image
    $this->Image($logo, 5, 3, $width, $height);
    ...
    // Company address
    $lines = $this->sizeOfText($adresse, $length);
...
function addLine($ligne, $tab)
{
    ...
    foreach ($colonnes as $lib => $pos) {
        ...
        $formText = $format[$lib];
        $this->SetXY($ordonnee, $ligne - 5);
        $this->Cell($longCell, 4, $texte, 0, 0, "C");
        $ordonnee += $longCell;
    }
}
